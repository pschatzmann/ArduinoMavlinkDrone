...
void SimpleMavlinkDrone::setMessageCallback(MessageCallback cb) {
  messageCallback = cb;
}

/**
 *  Constructor passing in a Stream which is used to read the data from. You can
 * also use your own implementation of the parameter store
 */
SimpleMavlinkDrone::SimpleMavlinkDrone(Stream* stream,
                                       ParameterStore* parameterStore) {
  log("SimpleMavlinkDrone");
  this->stream = stream;
  this->releaseParameterStore = parameterStore == NULL;
  this->parameterStore =
      (parameterStore != NULL) ? parameterStore : new ParameterStore();
}

SimpleMavlinkDrone::~SimpleMavlinkDrone() {
  log("~SimpleMavlinkDrone");
  if (releaseParameterStore) delete this->parameterStore;
}
...
void SimpleMavlinkDrone::loop() {
   ...
   * ==================== If bytes come in on the serial port, we feed them to
   * mavlink_parse_char(). This helper function keeps track of incoming bytes
   * and alerts us when it's received a complete, valid MAVLink message. See
   * https://github.com/mavlink/c_library_v2/blob/master/mavlink_helpers.h#L966
   *==============================================================================*/
   ...
   *
   *  In this code we:
   *  -Respond to initial messages sent from QGC to the vehicle when it first
   * connects, including: -A parameter list request. We send an arbitary float
   * parameter. -A request for "mission items." We say we have none. -A request
   * for autopilot capabilities and version. We make some things up. -Take
   * action on manual control joystick messages -- retransmitted back to QGC for
   * viewing in MAVLink inspector -Arm or disarm the vehicle and acknowledge
   * arm/disarm status based on incoming command messages. There may be more
   * messages you want to handle. The Message IDs below are defined in
   * individual message's .h files. For example:
   * https://github.com/mavlink/c_library_v2/blob/master/common/mavlink_msg_manual_control.h#L4
   *  You can find message numbers and field descriptions at
   ...
   * ====================== To let QGroundControl know a vehicle is present, we
   * send a heartbeat every 1s. Here I'm using the millis() function to schedule
   * outbound messages. You don't want to use blocking delay() calls in code
   * like this, because you want to read the incoming serial port as often as
   * possible.
   *
...
void SimpleMavlinkDrone::transmitMessage(mavlink_message_t* mvl_msg_ptr) {
  // log("transmitMessage");
  uint8_t tx_byte_buffer[512] = {
      0};  // A byte buffer that will be sent from the serial port.
...
// Unused axes can be disabled an buttons are also transmit as boolean values of
// their
void SimpleMavlinkDrone::handleManualControl(mavlink_message_t* mvl_msg_ptr) {
  log("handleManualControl");
  mavlink_manual_control_t mvl_joy;  // manual control data structure into which
                                     // we decode the message
  mavlink_msg_manual_control_decode(mvl_msg_ptr, &mvl_joy);

  // Values are normalized to the range [-1000,1000] -> we devide it by 10 to
  // convert it to %
  ...
  // For now, let's just retransmit the manual control message to see it in
  // MAVLink Inspector
...
void SimpleMavlinkDrone::handleParamRequestList(
    mavlink_message_t* mvl_msg_ptr) {
  ...
  mvl_param.param_type = MAV_PARAM_TYPE_REAL32;
  ...
  switch (mvl_cmd.command) {
    ...
    case (MAV_CMD_COMPONENT_ARM_DISARM): {
      ...
      mavlink_command_ack_t
          mvl_ack;  // https://mavlink.io/en/messages/common.html#COMMAND_ACK
      mvl_ack.command =
          MAV_CMD_COMPONENT_ARM_DISARM;  // https://mavlink.io/en/messages/common.html#MAV_CMD_COMPONENT_ARM_DISARM
      mvl_ack.result =
          MAV_RESULT_ACCEPTED;  // https://mavlink.io/en/messages/common.html#MAV_RESULT
...
void SimpleMavlinkDrone::handleMissionRequestList(
    mavlink_message_t* mvl_msg_ptr) {
...
void SimpleMavlinkDrone::processHeartbeat() {
  ...
  mavlink_heartbeat_t
      mvl_hb;  // struct with user fields: uint32_t custom_mode, uint8_t type,
               // uint8_t autopilot, uint8_t base_mode, uint8_t system_status;
  mvl_hb.type =
      MAV_TYPE_GENERIC;  // My vehicle is an underwater ROV. Change as
                         // appropriate. See:
                         // https://github.com/mavlink/c_library_v2/blob/748192f661d0df3763501cfc432861d981952921/common/common.h#L69
  mvl_hb.autopilot =
      MAV_AUTOPILOT_GENERIC;  // See
                              // https://github.com/mavlink/c_library_v2/blob/748192f661d0df3763501cfc432861d981952921/common/common.h#L40
  ...
  mvl_hb.base_mode |=
      MAV_MODE_FLAG_MANUAL_INPUT_ENABLED;  // We want to control the drone
...
void SimpleMavlinkDrone::processStat() {
  ...
  // accept each field as an argument instead of the mavlink_<message
  // name>_encode() that accepts a struct. They should save some memory to skip
  // the extra message struct, but I think setting each field by name in a demo
...
void SimpleMavlinkDrone::processStatSystem() {
  ...
  mavlink_sys_status_t
      mvl_sys_stat;  // #1 SYS_STATUS
                     // https://mavlink.io/en/messages/common.html#SYS_STATUS
  mvl_sys_stat.onboard_control_sensors_present =
      0;  // To set these, consult
  ...
  mvl_sys_stat.load = 0;
  mvl_sys_stat.voltage_battery =
      getValue(VOLTAGE_BATTERY) *
      1000;  // the only non-trivial telemetry we're sending, shows up several
             // places in QGC
...
void SimpleMavlinkDrone::setLog(Stream& log) { logStreamPtr = &log; }

void SimpleMavlinkDrone::log(const char* str) {
  if (logStreamPtr) logStreamPtr->println(str);
}
